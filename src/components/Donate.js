...
const Donate = () => {
	...
	const { isOpen: paymentisOpen, onOpen: paymentOpen, onClose: paymentClose } = useDisclosure();
  ...
  const [ ageError, setAgeError] = React.useState('');
	...
	return (
		<Flex gap={{ base: 3, md: 12 }} direction={{ base: 'column', md: 'row' }}>
			<Box w={{ base: '100%', md: '50%' }}>
				<Flex w={{ base: '100%', md: '100%' }} py={{ base: '20px', md: '80px' }} direction="column" gap={4}>
					...
					<Text
						...
						color="darkgreen"
					>
						Elections are expensive. They still cost billions of Naira. Osinbajo has never been a “money
						bag.” We need your help.
					</Text>
					<Text
						textAlign="justify"
						fontSize={{ base: '14px', md: '18px' }}
						...
						color="darkgreen"
					>
						We work every day investing in public and grassroot campaign infrastructure, taking YOUR message
						to delegates. None of this work is possible without your support.
					</Text>
					...
					<Text
						...
						color="darkgreen"
					>
						Please support us financially. Also let us know your email address – so we can thank you
						personally, and inform you of our activities from funds raised. Thank you for your support!
					</Text>
					<Text
						textAlign="justify"
						fontSize={{ base: '14px', md: '18px' }}
			...
			<Box w={{ base: '100%', md: '50%' }}>
				...
				>
					...
					<Flex direction="column" py={2} gap={1}>
						...
						/>
						{error !== '' && (
							<Text as="span" fontSize={{ base: '12px', md: '12px' }} fontWeight="400" color="red">
								{error}
							</Text>
						)}

						<Text as="span" fontSize={{ base: '12px', md: '14px' }} fontWeight="400" color="darkgreen">
					...
					<Flex direction="column" py={2} gap={1}>
						...
						<InputGroup>
							<InputLeftAddon children="+234">
								<Select border="none" onChange={(e) => setPhoneCode(e.target.value)}>
									{sortedCode.sort((a, b) => a - b).map(
										(line, index) =>
											line.dial_code === '+234' ? (
					...
					<Flex direction="column" py={2} gap={1}>
						...
						<Select placeholder="Select" onChange={(e) => {setAge(e.target.value); setAgeError('')}}>
			...
			<PaymentModal isOpen={paymentisOpen} onOpen={paymentOpen} onClose={paymentClose} pay={handleDonate} />
		</Flex>
	);
};
